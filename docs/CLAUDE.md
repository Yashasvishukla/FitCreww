...
- **1.2 Tenancy core** — Tenant/TenantConfig Prisma models, tenant-scoping Client Extension, RLS migration (raw SQL policies), the `withTenant(tenantId, callback)` interactive transaction wrapper that sets transaction-local `app.tenant_id`, app-role grants with `NOBYPASSRLS`, and seed data for two dev tenants. Acceptance: unit tests prove Prisma query rewriting; migration SQL creates schemas, tables, RLS, policy, grants, and app role; integration tests seed two tenants through the privileged URL, then prove the app URL can read only the selected tenant and raw SQL cannot cross tenants. *Demo: `pnpm --filter @fitcrew/db migrate:deploy && pnpm --filter @fitcrew/db seed && pnpm --filter @fitcrew/db test:integration` against local Postgres.*
- **1.3 Identity & auth** — platform-wide `user_account` plus Auth.js adapter tables, Argon2id credentials, JWT sessions for Auth.js Credentials compatibility, login Server Action, and protected route group. Middleware may only perform a cookie-presence redirect; every protected layout, Server Action, and Route Handler must call `auth()` server-side before reading or mutating data. Lock credentials after five failed attempts for 15 minutes; do not distinguish unknown, locked, or invalid accounts. *Demo: log in from the browser with the dev account, receive a session cookie, hit a protected Server Action; an unsigned-in request is redirected and a forged cookie is rejected by the server guard.*

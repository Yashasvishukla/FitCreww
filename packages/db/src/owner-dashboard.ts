...
export type OwnerEarningsRange = { from: Date; to: Date };

export async function getOwnerDashboard(client: PrismaClient, tenantId: string, userId: string, options?: { earningsRange?: OwnerEarningsRange }) {
  return withTenant(client as unknown as TransactionCapableClient<Tx>, tenantId, async (tx) => {
    ...
    const earningsFrom = day(options?.earningsRange?.from ?? month); const earningsTo = day(options?.earningsRange?.to ?? today);
    if (earningsTo < earningsFrom) throw new OwnerDashboardError('Earnings end date cannot be before the start date.');
    const earningsEndExclusive = addDays(earningsTo, 1);
    const [clients, sessions, recentSessions, activeSessionClients, payments, accruals, ownerEarningsAccruals, ownerEarningsOverallAccruals, schedules, subscriptions, coaches, organizations, satisfaction] = await Promise.all([
      ...
      tx.trainingSession.findMany({ where: { tenantId, sessionDate: { gte: thirtyDaysAgo, lt: addDays(today, 1) } }, select: { clientId: true } }),
      ...
      tx.commissionAccrual.findMany({ where: { tenantId, payment: { status: 'confirmed', confirmedAt: { gte: earningsFrom, lt: earningsEndExclusive } } }, select: { commissionAmount: true } }),
      tx.commissionAccrual.findMany({ where: { tenantId, payment: { status: 'confirmed' } }, select: { commissionAmount: true } }),
    ...
    const clientById = new Map(clients.map((client) => [client.id, client])); const assignmentByClient = new Map(assignmentRows.map((assignment) => [assignment.clientId, assignment])); const recentlyActiveClientIds = new Set(activeSessionClients.map((session) => session.clientId));
    ...
    const ownerEarningsForRange = ownerEarningsAccruals.reduce((sum, accrual) => sum + money(accrual.commissionAmount), 0);
    const ownerEarningsOverall = ownerEarningsOverallAccruals.reduce((sum, accrual) => sum + money(accrual.commissionAmount), 0);
    return { refreshedAt: now.toISOString(), period: { weekStarting: week.toISOString().slice(0, 10), monthStarting: month.toISOString().slice(0, 10), earningsFrom: earningsFrom.toISOString().slice(0, 10), earningsTo: earningsTo.toISOString().slice(0, 10) }, kpis: { activeClients: clients.length, sessionsThisWeek: sessions.length, revenueThisMonth: payments.reduce((sum, payment) => sum + money(payment.amount), 0), ownerEarningsForRange, ownerEarningsOverall, outstandingPayables: accruals.reduce((sum, accrual) => sum + money(accrual.coachPayableAmount), 0), evaluationsDue: schedules.length, activeOrganizations: organizations.length }, activity: { recentSessions: recentSessions.map((session) => ({ clientName: session.client.party.displayName, sessionDate: session.sessionDate.toISOString().slice(0, 10), startTime: session.startTime })) }, coaches: coachRows, organizations: orgRows, exceptions, drillDown: { clientCount: clientById.size } };
  });
}

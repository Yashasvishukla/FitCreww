...
export default async function OrganizationsPage({ searchParams }: { searchParams: { tenantId?: string } }) {
  ...
  if (!canCreate) {
    try { return <main className="dashboard-page organizations-page"><NetworkNav tenantId={tenantId} /><OrgDashboard data={await getOrgDashboardForUser(prisma, tenantId, session.user.id)} /></main>; } catch { return <main className="dashboard-page organizations-page"><NetworkNav tenantId={tenantId} /><AccessFallback tenantId={tenantId} eyebrow="Organizations" title="No organization access found" message="You are signed in, but this account is not assigned to an active organization in this workspace. Ask the workspace owner to connect your role to the correct organization." primaryHref="/profile" primaryLabel="Review account" /></main>; }
  }
    ...
    <main className="dashboard-page organizations-page">
      ...
      <div className="organizations-layout">
        <section className="surface organizations-list-panel">
          ...
          {loadError ? <AccessFallback tenantId={tenantId} eyebrow="Organizations" title="Organization list could not be loaded" message="The workspace is open, but the organization roster is unavailable right now. Try again, or check that your owner access is still active." primaryHref="/organizations" primaryLabel="Try again" embedded /> : organizations.length === 0 ? (
            <div className="organization-empty-state"><strong>Your partner network starts here.</strong><p>Create an organization to invite its administrator, define the agreement, and set up delivery coverage.</p></div>
          ) : (

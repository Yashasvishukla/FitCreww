  ...
  return <>
    <section className="dashboard-hero" aria-labelledby="dashboard-hero-title">
      ...
      <div className="hero-briefing" aria-label="Network snapshot">
        ...
        <div className={exceptionCount ? 'hero-briefing-alert' : 'hero-briefing-clear'}><span>{exceptionCount ? 'Needs attention' : 'All clear'}</span><strong>{exceptionCount || 0}</strong></div>
      </div>
    </section>
    ...
    <div className="dashboard-actions" aria-label="Quick actions">
      ...
      <Link className="action-pill" href={link('/coaches')}>Invite coach</Link>
      <Link className="action-pill" href={link('/organizations')}>Add organization</Link>
      <Link className="action-pill" href={link('/money')}>Record payment</Link>
    </div>
    ...
    <section className="surface attention-card" aria-labelledby="attention-title"><div className="section-heading"><div><p className="eyebrow">Action queue</p><h2 id="attention-title">{exceptionCount ? `${exceptionCount} items need your attention` : 'No action required today'}</h2></div>{exceptionCount ? <Link className="text-link" href={link('/dashboard/exceptions')}>Review all</Link> : null}</div>{exceptionCount ? <div className="exception-list">{data.exceptions.lapsedSubscriptions.length ? <Exception href={link('/dashboard/exceptions')} count={data.exceptions.lapsedSubscriptions.length} title="Subscriptions expired" detail="Renew or close" /> : null}{data.exceptions.overdueEvaluations.length ? <Exception href={link('/dashboard/exceptions')} count={data.exceptions.overdueEvaluations.length} title="Evaluations overdue" detail="Schedule a check-in" /> : null}{data.exceptions.inactiveClients.length ? <Exception href={link('/dashboard/exceptions')} count={data.exceptions.inactiveClients.length} title="Clients inactive" detail="No session in 30 days" /> : null}{data.exceptions.unsettledPayables.length ? <Exception href={link('/earnings')} count={data.exceptions.unsettledPayables.length} title="Coach payouts" detail="Ready to settle" /> : null}</div> : <p className="muted">No expired subscriptions, overdue evaluations, inactive clients, or coach payouts to review.</p>}</section>
...
function Exception({ href, count, title, detail }: { href: string; count: number; title: string; detail: string }) {
  return <Link href={href} className="exception-row"><strong>{count}</strong><span><b>{title}</b><small>{detail}</small></span><i aria-hidden="true">›</i></Link>;
}

...
export function ProfileForms({ tenantId, profile }: { tenantId: string; profile: Profile }) {
  ...
  return <div className="page-grid">
    ...
    <section className="surface profile-info-surface">
      ...
      <div className="profile-facts"><div><span>Role</span><strong>{profile.role}</strong></div>{profile.coach?.assignedOrganizationCount ? <div><span>Organization access</span><strong>{profile.coach.assignedOrganizationCount} {profile.coach.assignedOrganizationCount === 1 ? 'organization' : 'organizations'}</strong></div> : null}{!profile.coach && profile.organizations.length ? <div><span>Organizations</span><strong>{profile.organizations.length}</strong></div> : null}{profile.client ? <div><span>Client status</span><strong>{profile.client.status}</strong></div> : null}</div>
      ...
      {profile.coach && profile.coach.assignedOrganizationCount > 0 ? <div className="organization-directory">
        ...
        {visibleOrganizations.length > 0 ? <div className="profile-organization-list" aria-live="polite">{visibleOrganizations.map((org) => <div className="profile-organization-card" key={org.id}><div className="profile-organization-mark" aria-hidden="true">{initials(org.name)}</div><div className="profile-organization-card-copy"><strong title={org.name}>{org.name}</strong><span>{org.status} · ID ending {org.id.slice(-6)}</span></div><span className={`profile-organization-status profile-organization-status-${org.status.toLowerCase()}`}>{org.status}</span></div>)}</div> : <div className="directory-empty"><strong>No organizations found</strong><span>{organizationQuery ? 'Try a different search.' : 'No organization access is available in this workspace.'}</span></div>}

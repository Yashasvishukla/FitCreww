...
type Settlement = { id: string; coachName: string; periodStart: string; periodEnd: string; grossRevenue: string; commissionAmount: string; totalAmount: string; status: string; payoutStatus: 'pending' | 'confirmed' | 'reversed' | null; payslipMediaAssetId: string | null };
...
export function EarningsWorkspace({ tenantId, initial }: { tenantId: string; initial: Data }) {
  ...
  async function send(method: 'POST' | 'PATCH', body: object, action: string) {
    ...
    try {
      ...
      if (!response.ok) { setError(result.error ?? 'Settlement could not be updated.'); if (response.status === 409) router.refresh(); settleAction(action, 'failed'); return false; }
